    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <TouchableOpacity onPress={() => navigation.navigate('ASMDashboard')} style={styles.backBtn}>
            <Ionicons name="arrow-back" size={24} color="#0F172A" />
          </TouchableOpacity>
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>📋 Attendance & GPS Tracking</Text>
            <Text style={styles.subtitle}>Self GPS attendance punch-in & Team Attendance health tracking.</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {activeTab === 'TeamAttendance' && (
          <View>
            ...
            {/* MR Team Attendance & GPS Exception Health List */}
            <View style={[styles.card, { marginTop: 16 }]}>
              <Text style={styles.cardTitle}>👨‍💼 MR Attendance & GPS Exception Health</Text>

              {teamAttendanceData.map((item) => (
                <View key={item.id} style={styles.teamAttdCard}>
                  <View style={styles.teamAttdHeader}>
                    <View>
                      <Text style={styles.teamRsmName}>{item.mrName}</Text>
...
export default ASMAttendanceScreen;
