          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              ...
              {Platform.OS === 'web' ? (
                ...
                >
                  <Text style={styles.nativeBtnText}>{followUpDate ? formatDateDisplay(followUpDate) : 'Select Date'}</Text>
                </TouchableOpacity>
              )}
              {showFollowUpPicker && (
                <RNDateTimePicker
                  ...
                  onChange={(e, d) => {
                    ...
                    if (d) setFollowUpDate(d.toISOString().split('T')[0]);
                  }}
                />
              )}
            </View>
            ...
            {meetings.map((meet) => {
              ...
              return (
                <View key={meet.id} style={styles.meetCard}>
                  ...
                  <Text style={styles.meetInfo}>📅 Date: {formatDateDisplay(meet.date)} at {meet.time}</Text>
                  <Text style={styles.meetInfo}>📍 Venue: {meet.venue || 'Not Specified'} ({meet.meetingMode || 'Physical'})</Text>
                  <Text style={styles.meetInfo}>👤 Organizer: {meet.organizer || 'Not Specified'}</Text>
                  <Text style={styles.meetInfo}>👥 Participants ({meet.attendeesCount || 0}): {meet.participants}</Text>
                  {meet.followUpDate ? (
                    <Text style={styles.meetInfo}>🔁 Follow Up: {formatDateDisplay(meet.followUpDate)}</Text>
                  ) : null}
                  {meet.reminder && meet.reminder !== 'None' && (
                    <Text style={styles.meetInfo}>🔔 Reminder: {meet.reminder} before</Text>
                  )}
                  {meet.agenda ? (
                    <Text style={styles.meetInfo}>📝 Agenda: {meet.agenda}</Text>
                  ) : null}
