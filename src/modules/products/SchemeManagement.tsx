...
export default function SchemeManagement() {
  const [data, setData] = useState<Scheme[]>(initialMockData);
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="space-y-2">
                <DrawerField label="Scheme Code" value={<span className="font-mono text-violet-700 bg-violet-50 px-2 py-1 rounded">{selectedScheme.schemeCode}</span>} />
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-3">Applicability Information</h3>
              <div className="space-y-2">
                <DrawerField label="Applicable To" value={selectedScheme.applicableTo || '-'} />
                {selectedScheme.applicableTo !== 'All Products' && (
                  <DrawerField label="Product / Category / Brand" value={selectedScheme.applicableSelection || '-'} />
                )}
              </div>
            </div>

            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-3">Benefit Information</h3>
              <div className="space-y-2">
                <DrawerField label="Benefit Type" value={selectedScheme.benefitType} />
                <DrawerField label="Benefit Value" value={selectedScheme.benefitValue} />
              </div>
            </div>

            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-3">Quantity Criteria</h3>
              <div className="space-y-2">
                <DrawerField label="Min Purchase Qty" value={selectedScheme.minQuantity || 'N/A'} />
                <DrawerField label="Free Qty" value={selectedScheme.freeQuantity || 'N/A'} />
              </div>
            </div>

            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-3">Validity Information</h3>
              <div className="space-y-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-3">Additional & Status</h3>
              <div className="space-y-2">
                <DrawerField label="Remarks" value={selectedScheme.remarks || 'N/A'} />
                <DrawerField
                  label="Status"
                  value={
                    <Badge variant={selectedScheme.status === 'Active' ? 'success' : selectedScheme.status === 'Upcoming' ? 'info' : (selectedScheme.status === 'Draft' ? 'warning' : 'danger')}>
      ...
      {itemToDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-2">Delete Scheme</h3>
            <p className="text-sm text-slate-500 mb-6">
              Are you sure you want to delete this promotional scheme? This action cannot be undone.
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">{isEditingModal ? 'Edit Scheme' : 'Create Scheme'}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 mt-2 first:mt-0">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">SCHEME INFORMATION</h3>
              </div>
              ...
              <div className="md:col-span-2 mt-4">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">APPLICABILITY INFORMATION</h3>
              </div>
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    setNewScheme({ ...newScheme, applicableTo: value, applicableSelection: value === 'All Products' ? '' : newScheme.applicableSelection });
                  }} 
                  className="w-full border border-slate-200 rounded-lg px-3 py-2"
                >
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Product / Category / Brand Selection</label>
                <input 
                  type="text"
                  ...
                  placeholder={newScheme.applicableTo === 'All Products' ? 'Not Applicable' : `Select ${newScheme.applicableTo}...`}
                />
              </div>

              {/* BENEFIT INFORMATION */}
              <div className="md:col-span-2 mt-4">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">BENEFIT INFORMATION</h3>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Benefit Type *</label>
                <select 
                  value={newScheme.benefitType} 
                  onChange={(e) => setNewScheme({ ...newScheme, benefitType: e.target.value })} 
                  className="w-full border border-slate-200 rounded-lg px-3 py-2"
                >
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={(e) => setNewScheme({ ...newScheme, benefitValue: e.target.value })} 
                  className="w-full border border-slate-200 rounded-lg px-3 py-2" 
                  placeholder="e.g. 5"
                />
              </div>
              ...
              {/* QUANTITY CRITERIA */}
              <div className="md:col-span-2 mt-4">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">QUANTITY CRITERIA</h3>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Minimum Purchase Quantity</label>
                <input 
                  type="number"
                  value={newScheme.minQuantity} 
                  onChange={(e) => setNewScheme({ ...newScheme, minQuantity: e.target.value })} 
                  className="w-full border border-slate-200 rounded-lg px-3 py-2" 
                  placeholder="e.g. 10"
                />
              </div>
              <div>
                ...
                <input 
                  ...
                  onChange={(e) => setNewScheme({ ...newScheme, freeQuantity: e.target.value })} 
                  className="w-full border border-slate-200 rounded-lg px-3 py-2" 
                  placeholder="e.g. 1"
                />
              </div>
              ...
              <div className="md:col-span-2 mt-4">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">VALIDITY INFORMATION</h3>
              </div>
              <div>
              ...
              <div className="md:col-span-2 mt-4">
                <h3 className="text-sm font-semibold text-slate-700 border-b pb-2 mb-2">ADDITIONAL & STATUS INFORMATION</h3>
              </div>
              <div className="md:col-span-2">
            ...
            <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-slate-100">
              ...
              <ActionButton onClick={handleSaveScheme}>{isEditingModal ? 'Save Changes' : 'Create Scheme'}</ActionButton>
            </div>
          </div>
        </div>
      )}
