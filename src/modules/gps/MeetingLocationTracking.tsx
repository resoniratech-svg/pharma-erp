...
export default function MeetingLocationTracking() {
  ...
  const totalAttendees = meetings.reduce((sum, m) => sum + (m.attendees || 0), 0);
    ...
    <div className="animate-in fade-in duration-500 min-h-[calc(100vh-140px)] flex flex-col">
      <PageHeader
        ...
        actions={
          <ActionButton 
            ...
            onClick={() => alert("CSV Export feature will be enabled once backend API is integrated.")}
          >
            Export Data
          </ActionButton>
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <SummaryCard
          ...
          subtitle="Require manual GPS review"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-8 flex-1">
        <div className="xl:col-span-2 flex flex-col gap-4">
          <FilterBar>
            <SearchInput value={search} onChange={setSearch} placeholder="Search event, location, organizer, or ID..." />
            <div className="w-px h-6 bg-slate-200 mx-2 hidden sm:block" />
            <div className="flex items-center gap-2">
              <Filter className="w-4 h-4 text-slate-400" />
              <span className="text-sm font-medium text-slate-600">Filters:</span>
          ...
          <TableCard>
            <DataTable
              columns={columns}
              data={filteredData}
              emptyMessage="No meeting locations available. Meeting records will appear here after events are created."
            />
          </TableCard>
        </div>
