...
import { Download, Users, Eye, Map, Share2, Info, Loader2 } from 'lucide-react';
...
export default function SalesOrganization() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const reportingRsms = await nsmService.getReportingRSMs();
      setRsms(reportingRsms);
    } catch (e: any) {
      console.warn('Failed to load reporting managers:', e);
      setError(e.message || 'Failed to load reporting managers');
    } finally {
      setLoading(false);
    }
  };

  const filteredData = rsms.filter(row => 
  ...
  const columns = [
    { key: 'employeeCode', label: 'Employee Code', render: (row: Employee) => row.employeeCode || `EMP-${row.id}` },
  ...
  return (
    <div className="p-6">
      <PageHeader 
        ...
        subtitle="Live view of the national sales hierarchy (Database Integrated)."
        actions={
          <ActionButton variant="secondary" icon={<Download className="w-4 h-4" />} onClick={() => exportToCSV(filteredData, columns as any, 'NSM_Sales_Organization.csv')}>
            Export Directory
          </ActionButton>
        }
      />
      ...
      <TableCard>
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center text-slate-500">
            ...
            <p className="text-sm">Loading reporting sales hierarchy from database...</p>
          </div>
        ) : (
          <DataTable columns={columns} data={filteredData} emptyMessage="No reporting employees found in database." />
        )}
      </TableCard>
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                <DrawerField label="Employee Code" value={selectedEmployee.employeeCode || `EMP-${selectedEmployee.id}`} />
          ...
          <div className="space-y-6">
            ...
            {subordinates.length > 0 ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="bg-slate-50 border border-slate-200 border-dashed shadow-sm p-3 rounded-xl text-center w-full max-w-sm">
                  <p className="text-sm text-slate-500 italic">No direct subordinates found in database.</p>
                </div>
              </div>
            )}
            ...
            <div className="mt-4 p-4 bg-amber-50 border border-amber-100 rounded-xl flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800">
                This is a read-only view. Employee hierarchy is managed through the central Sales Organization module.
              </p>
            </div>
          </div>
        )}
