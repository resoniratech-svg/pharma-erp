...
const MOCK_TEAM_ATTENDANCE = [
  {
    ...
    status: 'Present',
    gpsStatus: 'Verified',
    checkInCoords: '19.1136° N, 72.8697° E',
    checkOutCoords: '19.0596° N, 72.8295° E',
    checkInAddress: 'Andheri West, Mumbai',
    checkOutAddress: 'Bandra East, Mumbai',
    deviceName: 'Samsung Galaxy S23',
    deviceId: 'DEV-892347',
    remarks: 'Field work as planned.'
  },
  {
    id: 'ATT002',
    date: '2026-08-02',
  ...
  },
  {
    ...
    status: 'Present',
    gpsStatus: 'Verified',
    checkInCoords: '23.0225° N, 72.5714° E',
    checkOutCoords: '23.0225° N, 72.5714° E',
    checkInAddress: 'Navrangpura, Ahmedabad',
    checkOutAddress: 'Navrangpura, Ahmedabad',
...
export default function Attendance() {
  ...
  const totalEmployees = MOCK_TEAM_ATTENDANCE.length;
  const presentCount = MOCK_TEAM_ATTENDANCE.filter(r => r.status === 'Present').length;
  const lateCount = MOCK_TEAM_ATTENDANCE.filter(r => r.status === 'Late').length;
  const absentCount = MOCK_TEAM_ATTENDANCE.filter(r => r.status === 'Absent' || r.status === 'On Leave').length;
  
  // Treat 'Present' and 'Late' and 'Half Day' as some form of attendance for the overall percentage
  const totalAttended = presentCount + lateCount + MOCK_TEAM_ATTENDANCE.filter(r => r.status === 'Half Day').length;
          ...
          <div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 space-y-4">
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <ActionButton variant="secondary" icon={<Download className="w-4 h-4" />} onClick={() => exportToCSV(filteredData, columns as any, 'Team_Attendance.csv')}>
                  Export
                </ActionButton>
              </div>
            </div>
