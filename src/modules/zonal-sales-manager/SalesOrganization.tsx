...
import { Users, Eye, Map, Share2, Info } from 'lucide-react';
...
export default function SalesOrganization() {
  ...
  useEffect(() => {
    try {
      const reportingRsms = zsmService.getReportingRSMs();
      setRsms(reportingRsms);
    } catch (e) {
      console.warn('Failed to load RSMs:', e);
    }
  }, []);
  ...
  return (
    <div className="p-6">
      <PageHeader 
        title="Zone Sales Organization" 
        subtitle="Read-only view of Regional Sales Managers reporting to you."
      />
      ...
      <TableCard>
        <DataTable columns={columns} data={filteredData} emptyMessage="No reporting RSMs found." />
      </TableCard>
