...
import type { ZSMTargetSummary } from '../../services/zsmService';
...
export default function TargetAllocation() {
  const [summaries, setSummaries] = useState<ZSMTargetSummary[]>([]);
  ...
  const refreshData = () => {
    try {
      setSummaries(zsmService.getTargetSummaries());
      setRsms(zsmService.getReportingRSMs());
    } catch (e) {
      console.warn('Failed to load ZSM allocations data', e);
    }
  };

  const filteredTargets = summaries.filter(s => {
    const row = s.parentAllocation;
    const targetCode = s.parentTargetCode || '';
    const rowId = row.id || '';
    const codeMatches = targetCode.toLowerCase().includes(search.toLowerCase()) || rowId.toLowerCase().includes(search.toLowerCase());
  ...
  const handleOpenAllocate = () => {
    ...
    setAllocationRows([{ rsmId: '', amount: '', financialYear: selectedSummary.parentAllocation.financialYear || '', allocationPeriod: '', startDate: '', endDate: '', remarks: '' }]);
    setDrawerOpen(true);
  };

  const handleAddAllocationRow = () => {
    setAllocationRows([...allocationRows, { rsmId: '', amount: '', financialYear: selectedSummary?.parentAllocation.financialYear || '', allocationPeriod: '', startDate: '', endDate: '', remarks: '' }]);
  };
  ...
  const handleCancelAllocation = (allocId: string) => {
    ...
    try {
      zsmService.cancelAllocation(allocId);
      refreshData();
    } catch (e: any) {
  ...
  const handleSaveAllocation = () => {
    ...
    try {
      if (editAllocationId) {
        ...
        zsmService.updateAllocation(editAllocationId, Number(row.amount), row.rsmId);
      } else {
  ...
  const targetColumns = [
    ...
    { key: 'id', label: 'Received Allocation ID', render: (row: ZSMTargetSummary) => <span className="font-medium text-[#163c78]">{row.parentAllocation.id}</span> },
    { key: 'year', label: 'Financial Year', render: (row: ZSMTargetSummary) => row.parentAllocation.financialYear },
    { 
      key: 'totalTarget', 
      label: 'Received Amount',
      render: (row: ZSMTargetSummary) => <span className="font-bold text-slate-800">₹{(row.parentAllocation.targetAmount / 100000).toFixed(2)} L</span>
    },
    { 
      key: 'allocatedTarget', 
      label: 'Allocated Down',
      render: (row: ZSMTargetSummary) => `₹${(row.allocatedAmount / 100000).toFixed(2)} L`
    },
    { 
      key: 'remainingTarget', 
  ...
  const allocationColumns = [
    ...
    { key: 'rsm', label: 'Regional Sales Manager', render: (row: TargetAllocationRecord) => <span className="font-semibold text-slate-800">{row.allocatedToEmployeeName}</span> },
  ...
  return (
    <div className="p-6">
      <PageHeader 
        ...
        subtitle="Allocate Targets received from the NSM down to your Regional Sales Managers."
        actions={
          <ActionButton 
            variant="primary" 
      ...
      <div className="mb-6 bg-blue-50/50 border border-blue-200 rounded-xl p-4 flex gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-blue-600 mt-1">
            Select a target assignment (allocated to you by the NSM) from the list below to view its details or allocate it further to Regional Sales Managers. Total allocation cannot exceed your remaining assigned balance.
          </p>
        </div>
      </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-slate-800">My Assigned Targets (from NSM)</h2>
        </div>
        <FilterBar>
          <SearchInput value={search} onChange={setSearch} placeholder="Search allocation ID or master code..." />
          <SelectFilter 
            value={statusFilter} 
        ...
        <TableCard>
          <DataTable 
            ...
            emptyMessage="No active targets have been assigned to you by the NSM."
          />
        </TableCard>
      </div>
        ...
        {selectedRowId ? (
          <TableCard>
            <DataTable 
              ...
              emptyMessage="No allocations have been made to your RSMs for this target yet." 
            />
          </TableCard>
        ) : (
      ...
      <Modal 
        ...
        title={editAllocationId ? "Edit Allocation" : "Allocate Target to Regional Sales Managers"}
        footer={
          <>
        ...
        {selectedSummary && (
          <div className="space-y-6 flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-slate-800">{editAllocationId ? "Edit Row" : "RSM Allocations"}</h3>
                {!editAllocationId && (
                ...
                {allocationRows.map((row, index) => (
                  <div key={index} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm relative">
                    ...
                    <div className="grid gap-4 pr-6">
                      <div>
                        <label className="block text-xs font-medium text-slate-500 mb-1">Select Regional Sales Manager *</label>
                        <select 
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedSummary.parentTargetCode && (
                  <DrawerField label="Master Target Code" value={selectedSummary.parentTargetCode} />
                )}
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <DrawerField label="Regional Sales Manager" value={selectedAllocation.allocatedToEmployeeName} />
