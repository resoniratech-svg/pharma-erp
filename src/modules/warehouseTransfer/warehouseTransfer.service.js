...
const createTransferService = async (data) => {
  ...
  // Loop through items to validate and transfer stock
  for (const item of items) {
    ...
    if (!sourceInventory) {
      throw new Error("Source inventory not found for a batch");
    }

    if (sourceInventory.quantity < quantity) {
      throw new Error("Insufficient stock for a batch");
    }
  ...
  return repository.createTransferRepo(data);
};
